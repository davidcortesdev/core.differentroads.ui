...
export class MainComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    // Usar requestAnimationFrame para esperar el siguiente frame de renderizado
    // sin usar setTimeout
    requestAnimationFrame(() => {
      this.initializeTermly();
    });
  }

  private initializeTermly(): void {
    ...
    if (win.Termly) {
      if (typeof win.Termly.scan === 'function') {
        win.Termly.scan();
      } else if (typeof win.Termly.init === 'function') {
        win.Termly.init();
      } else if (typeof win.Termly.refresh === 'function') {
        win.Termly.refresh();
      }
    }
