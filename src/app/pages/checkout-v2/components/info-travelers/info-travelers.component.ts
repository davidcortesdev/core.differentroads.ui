...
export class InfoTravelersComponent implements OnInit, OnDestroy, OnChanges {
  ...
  /**
   * Obtiene el valor de un campo desde el formulario reactivo
   */
  private getValueFromReactiveForm(travelerId: number, fieldCode: string): string {
    const controlName = `${fieldCode}_${travelerId}`;
    
    // Buscar en todos los formularios de viajeros
    for (let i = 0; i < this.travelerForms.length; i++) {
      const travelerForm = this.getTravelerForm(i);
      if (travelerForm) {
        const control = travelerForm.get(controlName);
  ...
  getLeaderTravelerInfo(): { name: string; lastName: string } {
    ...
    if (nameField) {
      ...
      // Si no hay valor en BD, intentar leer del formulario reactivo
      if (!name) {
        name = this.getValueFromReactiveForm(leaderTraveler.id, nameField.code);
      }
    }
    ...
    if (lastNameField) {
      ...
      // Si no hay valor en BD, intentar leer del formulario reactivo
      if (!lastName) {
        lastName = this.getValueFromReactiveForm(leaderTraveler.id, lastNameField.code);
      }
    }

    // Si no encontramos con los códigos exactos, buscar de manera más amplia
    if (!name || !lastName) {
      // Buscar por nombre del campo
      const nameFieldByLabel = this.reservationFields.find(f => 
        f.name.toLowerCase().includes('nombre') && 
        !f.name.toLowerCase().includes('apellido')
      );
      
      const lastNameFieldByLabel = this.reservationFields.find(f => 
        f.name.toLowerCase().includes('apellido') ||
        f.name.toLowerCase().includes('lastname') ||
        f.name.toLowerCase().includes('surname')
      );

      if (nameFieldByLabel && !name) {
        name = this.getExistingFieldValue(leaderTraveler.id, nameFieldByLabel.id);
        
        // Si no hay valor en BD, intentar del formulario
        if (!name) {
          name = this.getValueFromReactiveForm(leaderTraveler.id, nameFieldByLabel.code);
        }
      }

      if (lastNameFieldByLabel && !lastName) {
        lastName = this.getExistingFieldValue(leaderTraveler.id, lastNameFieldByLabel.id);
        
        // Si no hay valor en BD, intentar del formulario
        if (!lastName) {
          lastName = this.getValueFromReactiveForm(leaderTraveler.id, lastNameFieldByLabel.code);
        }
      }
    }
